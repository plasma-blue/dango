...
function render() {
    ...
    // Auto-save on render (simple approach)
    saveData();
}
...
function renderNode(el, node) {
    ...
    if (el.innerText !== node.text && !el.isContentEditable)
    ...
    // Store dimensions for alignment calc
    if (!node.w || !node.h || el.offsetWidth !== node.w) {
        // Quick update if rendered size changes
...
document.getElementById("btn-add").onclick = () => {
    ...
    const startX =
        -state.view.x / state.view.scale +
        window.innerWidth / (2 * state.view.scale);
    const startY =
        -state.view.y / state.view.scale +
        window.innerHeight / (2 * state.view.scale);
    let count = 0;
    parts.forEach((str, idx) => {
        if (!existingTexts.has(str)) {
            // Slight randomness to avoid stacking perfectly if multiple adds
            state.nodes.push({
                id: uid(),
                text: str,
                x: startX + (count % 5) * 140,
                y: startY + Math.floor(count / 5) * 80,
                w: 0,
                h: 0, // Init size 0, updated on render
                color: "c-white",
            });
            count++;
        }
...
btnClear.onclick = () => {
    if (!clearConfirm) {
        clearConfirm = true;
        btnClear.innerText = "确定?";
        btnClear.classList.add("btn-danger");
        // Reset if not clicked within 3s
        setTimeout(() => {
            if (clearConfirm) {
                clearConfirm = false;
    ...
    if (e.button === 0) {
        ...
        if (nodeEl) {
            ...
            dragStart = {
                x: worldPos.x,
                y: worldPos.y,
                initialPos: getSelectionPositions(),
            };
        } else if (groupEl) {
        ...
        state.selection.forEach((id) => {
            ...
            if (init) {
                ...
                if (item) {
                    ...
                    item.x = init.x + dx;
                    item.y = init.y + dy;
                    // If it's a group, we must update its members visually too (logic inside render handles basic translate,
                    // but we need to update actual member coordinates if we want data consistency)
                    if (init.type === "group") {
                        item.memberIds.forEach((mid) => {
                            const member = state.nodes.find((n) => n.id === mid);
                            if (member && !dragStart.initialPos[mid]) {
                                // Only move member if it wasn't selected individually (prevent double move)
...
els.container.addEventListener("mouseup", (e) => {
    if (mode === "box") {
        ...
        [...state.nodes, ...state.groups].forEach((item) => {
            if (isIntersect(worldRect, item)) state.selection.add(item.id);
        });
...
window.addEventListener("keydown", (e) => {
    ...
    if (e.altKey && !e.ctrlKey) {
        switch (e.code) {
            ...
            case "KeyH":
                e.preventDefault();
                e.shiftKey ? distributeSelection("h") : alignSelection("centerX");
                break;
            case "KeyV":
                e.preventDefault();
                e.shiftKey ? distributeSelection("v") : alignSelection("centerY");
                break;
        }
    }
});
...
function isIntersect(r1, r2) {
    const r2w = r2.w || 60;
    const r2h = r2.h || 40; // Fallback size
    return !(
        r2.x > r1.x + r1.w ||
        r2.x + r2w < r1.x ||
        r2.y > r1.y + r1.h ||
...
function toggleLink() {
    ...
    if (existingIdx !== -1) state.links.splice(existingIdx, 1);
    else state.links.push({ id: uid(), sourceId: n1.id, targetId: n2.id });
    render();
}
function deleteSelection() {
    ...
    state.links = state.links.filter(
        (l) => !sel.has(l.sourceId) && !sel.has(l.targetId)
    );
    state.groups.forEach((g) => {
        g.memberIds = g.memberIds.filter((mid) =>
            state.nodes.find((n) => n.id === mid)
        );
    });
    state.selection.clear();
    render();
...
// Updates item position and syncs group members if applicable
...
function distributeSelection(axis) {
    const items = [...state.selection].map((id) => findItem(id)).filter((i) => i);
    if (items.length < 3) return; // Need at least 3 to distribute

    // Sort items by coordinate
    if (axis === "h") items.sort((a, b) => a.x - b.x);
    ...
    if (axis === "h") {
        const totalSpan = last.x - (first.x + (first.w || 0)); // Span from first right edge to last left edge?? No, standard distribute centers or spaced.
        // Let's do "Distribute Horizontally" (Equal gap between elements)
        // Available space calculation:
        const totalWidth = last.x + (last.w || 0) - first.x;
        const sumItemWidths = items.reduce((sum, i) => sum + (i.w || 0), 0);
        const totalGap = last.x + (last.w || 0) - first.x - sumItemWidths; // Wait, standard distribute matches centers or gaps?
        // PPT "Distribute Horizontally" usually equalizes the distance between centers OR edges.
        // Let's implement: Equalize Gaps.

        // Range: First Left to Last Right.
        const startX = first.x;
        const endX = last.x + (last.w || 0);
        const totalSpace = endX - startX;
        const itemsTotalW = items.reduce((acc, it) => acc + (it.w || 0), 0);
        const gap = (totalSpace - itemsTotalW) / (items.length - 1);

        let currentX = startX;
        items.forEach((item) => {
            setItemPos(item, currentX, item.y);
            currentX += (item.w || 0) + gap;
        });
    } else {
        // Vertical
        const startY = first.y;
        const endY = last.y + (last.h || 0);
        const totalSpace = endY - startY;
        const itemsTotalH = items.reduce((acc, it) => acc + (it.h || 0), 0);
        const gap = (totalSpace - itemsTotalH) / (items.length - 1);

        let currentY = startY;
        items.forEach((item) => {
            setItemPos(item, item.x, currentY);
            currentY += (item.h || 0) + gap;
        });
    }
    render();
}
